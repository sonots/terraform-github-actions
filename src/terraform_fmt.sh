...
function terraformFmt {
  ...
  fmtOutput=$(${tfCommand} fmt -check=true -write=false -diff ${fmtRecursive} ${*} 2>&1)
  fmtExitCode=${?}
  ...
  fmtFileList=$(${tfCommand} fmt -check=true -write=false -list ${fmtRecursive})
  echo "${fmtFileList}"
  echo
  ...
  if [ "$GITHUB_EVENT_NAME" == "pull_request" ] && [ "${tfComment}" == "1" ]; then
    ...
    for file in ${fmtFileList}; do
      fmtFileDiff=$(${tfCommand} fmt -check=true -write=false -diff "${file}" | sed -n '/@@.*/,//{/@@.*/d;p}')
...
\`\`\`
    ...
    fmtCommentWrapper="#### \`${tfCommand} fmt\` Failed
${fmtComment}
